...
func Init(cfg *Config) error {
	...
	logDir := "data"
	// Fix G301: use 0700 for directories
	if err := os.MkdirAll(logDir, 0700); err == nil {
		// Fix G304: sanitize log filename
		safeName := filepath.Base(fmt.Sprintf("nofx_%s.log", time.Now().Format("2006-01-02")))
		logFileName := filepath.Join(logDir, safeName)
		// Fix G302: use 0600 for files
