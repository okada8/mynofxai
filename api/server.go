...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		api.GET("/ws/terminal", s.handleTerminalWebSocket)
		...
		// NofxOS API Proxy (Public, as DataPage is public)
		api.GET("/nofxos/*path", s.handleNofxosProxy)
		api.GET("/debug/nofxos", s.handleDebugNofxos)
		...
		api.POST("/test-notification", s.handleTestNotification) // Test notification endpoint
...
// handleNofxosProxy Proxy requests to NofxOS API
func (s *Server) handleNofxosProxy(c *gin.Context) {
	path := c.Param("path")
	query := c.Request.URL.RawQuery

	// Get API Key from config
	cfg := config.Get()
	apiKey := cfg.NofxosAPIKey
	if apiKey == "" {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "NofxOS API Key not configured"})
		return
	}

	// Build target URL
	targetURL := fmt.Sprintf("https://nofxos.ai%s", path)
	if query != "" {
		targetURL = fmt.Sprintf("%s?%s&auth=%s", targetURL, query, apiKey)
	} else {
		targetURL = fmt.Sprintf("%s?auth=%s", targetURL, apiKey)
	}

	// Create request
	req, err := http.NewRequest(c.Request.Method, targetURL, c.Request.Body)
	if err != nil {
		SafeInternalError(c, "Create proxy request", err)
		return
	}
	...
	if err != nil {
		SafeInternalError(c, "Execute proxy request", err)
		return
	}
	defer resp.Body.Close()

	// Copy response headers (except some specific ones)
	for k, v := range resp.Header {
		if k != "Content-Length" && k != "Access-Control-Allow-Origin" {
			for _, val := range v {
...
// handleDebugNofxos Debug NofxOS response
func (s *Server) handleDebugNofxos(c *gin.Context) {
	cfg := config.Get()
	apiKey := cfg.NofxosAPIKey
	if apiKey == "" {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "NofxOS API Key not configured"})
	...
	url := fmt.Sprintf("https://nofxos.ai/dashboard?auth=%s", apiKey)
	...
	// Search for inline fetch/axios
	reFetch := regexp.MustCompile(`fetch\(['"]([^'"]+)['"]`)
	fetchMatches := reFetch.FindAllStringSubmatch(string(body), -1)
	
	var fetches []string
	for _, m := range fetchMatches {
		if len(m) > 1 {
			fetches = append(fetches, m[1])
		}
	}
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	s.logAudit(c, "CREATE_TRADER", traderID, "SUCCESS", req)
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	s.logAudit(c, "UPDATE_TRADER", traderID, "SUCCESS", req)
...
func (s *Server) handleDeleteTrader(c *gin.Context) {
	...
	s.logAudit(c, "DELETE_TRADER", traderID, "SUCCESS", nil)
...
func (s *Server) handleStartTrader(c *gin.Context) {
	...
	s.logAudit(c, "START_TRADER", traderID, "SUCCESS", nil)
...
func (s *Server) handleStopTrader(c *gin.Context) {
	...
	s.logAudit(c, "STOP_TRADER", traderID, "SUCCESS", nil)
...
func (s *Server) handleUpdateModelConfigs(c *gin.Context) {
	...
	s.logAudit(c, "UPDATE_MODEL_CONFIG", "system", "SUCCESS", nil)
...
func (s *Server) handleUpdateExchangeConfigs(c *gin.Context) {
	...
	s.logAudit(c, "UPDATE_EXCHANGE_CONFIG", "system", "SUCCESS", nil)
...
// handleTerminalWebSocket WebSocket handler for terminal
func (s *Server) handleTerminalWebSocket(c *gin.Context) {
	...
	// Set initial window size (optional, can be dynamic if frontend sends resize events)
	...
	// Goroutine to copy output from pty to websocket
	go func() {
		defer close(done)
		buf := make([]byte, 1024)
		for {
			...
			// Send as TextMessage because xterm.js handles strings well
	...
	// Copy input from websocket to pty
	for {
	...
	// Wait for writer to finish or process to exit
	// We could also kill the process here if loop exits
	if cmd.Process != nil {
		cmd.Process.Kill()
	}
	<-done
...
func (s *Server) authMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if apiKey != "" {
			...
			keyRecord, err := s.store.APIKey().GetByHash(hash)
			if err == nil && keyRecord != nil {
				// Valid API Key
				c.Set("user_id", keyRecord.UserID)
				// Update usage time async
				go s.store.APIKey().UpdateLastUsed(keyRecord.ID)
				c.Next()
				return
			}
			// Invalid API Key - fail immediately if header is present
		...
		if authHeader == "" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "Missing Authorization header or X-API-KEY"})
			c.Abort()
			return
		}
...
// handleTestNotification Send a test notification via Telegram
func (s *Server) handleTestNotification(c *gin.Context) {
	err := notification.SendTelegramMessage("🔔 This is a test notification from NOFX Terminal.")
	if err != nil {
		logger.Errorf("Failed to send test notification: %v", err)
...
func (s *Server) Start() error {
	...
	logger.Infof("  • GET  /api/audit-logs       - System audit logs")
	logger.Info()
