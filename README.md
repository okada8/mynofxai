...
- **基础数据**: [数据 API](docs/data/README.md) | [指标 API](docs/indicators/README.md)
- **核心交易**: [策略 API](docs/strategy/README.md) | [回测 API](docs/backtest/README.md) | [看板与持仓](docs/dashboard/README.md)
- **高级功能**: [AI 辩论场](docs/arena/README.md) | [Polymarket](docs/polymarket/README.md) | [排行榜](docs/leaderboard/README.md)
- **系统管理**: [配置管理](docs/config/README.md) | [任务调度](docs/tasks/README.md) | [系统监控](docs/monitor/README.md)

## 🏗️ 系统架构 (Architecture)

NOFX 采用模块化分层架构设计，确保高扩展性与稳定性：

1. **接入层 (Interface Layer)**:
   - **REST API**: 基于 Gin 框架，提供标准化的 HTTP 接口。
   - **WebSocket**: 处理实时行情推送与日志流。
   - **Frontend**: React + TypeScript 构建的 SPA 应用。

2. **核心层 (Kernel Layer)**:
   - **Strategy Engine**: 负责策略解析、信号生成与多代理协作。
   - **Debate Arena**: 多 LLM 辩论引擎，通过角色扮演达成共识。
   - **Backtest Engine**: 高性能回测系统，支持历史数据重放。

3. **执行层 (Execution Layer)**:
   - **Trader Manager**: 管理多个交易实例（Trader），支持多交易所适配。
   - **Order System**: 统一订单管理，支持限价、市价及高级委托。

4. **基础设施层 (Infrastructure)**:
   - **Market Providers**: 统一行情数据接口 (Binance, CCXT, CoinAnk)。
   - **MCP (Model Context Protocol)**: AI 模型适配层，兼容 OpenAI, Claude, DeepSeek 等。
   - **Storage**: SQLite/PostgreSQL 数据持久化。
