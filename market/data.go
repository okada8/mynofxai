...
func calculateTimeframeSeries(klines []Kline, timeframe string, count int) *TimeframeSeriesData {
	...
	data := &TimeframeSeriesData{
		...
		ADX14Values: make([]float64, 0, count),
		VWAPValues:  make([]float64, 0, count),
		OBVValues:   make([]float64, 0, count),
		CMF20Values: make([]float64, 0, count),
	...
	for i := start; i < len(klines); i++ {
		...
		// Calculate ADX14 for each point
		if i >= 27 { // ADX14 needs roughly 2*period
			adx14 := calculateADX(klines[:i+1], 14)
			data.ADX14Values = append(data.ADX14Values, adx14)
		}

		// Calculate VWAP (rolling 20)
		if i >= 19 {
			vwap := calculateVWAP(klines[:i+1], 20)
		...
		// Calculate CMF20
		if i >= 19 {
			cmf20 := calculateCMF(klines[:i+1], 20)
			data.CMF20Values = append(data.CMF20Values, cmf20)
		}
...
func formatTimeframeData(sb *strings.Builder, data *TimeframeSeriesData) {
	...
	if len(data.ADX14Values) > 0 {
		sb.WriteString(fmt.Sprintf("ADX14: %s\n", formatFloatSlice(data.ADX14Values)))
	}
	...
	if len(data.CMF20Values) > 0 {
		sb.WriteString(fmt.Sprintf("CMF20: %s\n", formatFloatSlice(data.CMF20Values)))
	}
...
func calculateADX(klines []Kline, period int) float64 {
	...
	// Subsequent values
	dxList := make([]float64, len(klines))
	...
	if smoothTR != 0 {
		...
		if diSum != 0 {
			dxList[period] = (math.Abs(plusDI - minusDI) / diSum) * 100
		}
	}
	...
	for i := period + 1; i < len(klines); i++ {
		...
		if smoothTR != 0 {
			...
			diSum := plusDI + minusDI
			if diSum != 0 {
				dxList[i] = (math.Abs(plusDI - minusDI) / diSum) * 100
			}
		}
	}
...
// calculateVWAP calculates Volume Weighted Average Price (Rolling)
...
func ExportCalculateADX(klines []Kline, period int) float64 {
	return calculateADX(klines, period)
}

// ExportCalculateVWAP exports calculateVWAP for testing
func ExportCalculateVWAP(klines []Kline, period int) float64 {
	return calculateVWAP(klines, period)
}

// ExportCalculateOBV exports calculateOBV for testing
func ExportCalculateOBV(klines []Kline) float64 {
	return calculateOBV(klines)
}

// ExportCalculateCMF exports calculateCMF for testing
func ExportCalculateCMF(klines []Kline, period int) float64 {
	return calculateCMF(klines, period)
}
