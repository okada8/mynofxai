...
func (e *StrategyEngine) writeAvailableIndicators(sb *strings.Builder) {
	...
	if indicators.EnableADX {
		sb.WriteString("- ADX indicator (14-period)\n")
	}

	if indicators.EnableVWAP {
		sb.WriteString("- VWAP indicator (20-period rolling)\n")
	}

	if indicators.EnableOBV {
		sb.WriteString("- On-Balance Volume (OBV)\n")
	}

	if indicators.EnableCMF {
		sb.WriteString("- Chaikin Money Flow (CMF) (20-period)\n")
	}
...
func (e *StrategyEngine) formatTimeframeSeriesData(sb *strings.Builder, data *market.TimeframeSeriesData, indicators store.IndicatorConfig) {
	...
	if indicators.EnableADX && len(data.ADX14Values) > 0 {
	...
	if indicators.EnableOBV && len(data.OBVValues) > 0 {
		sb.WriteString(fmt.Sprintf("OBV: %s\n", formatFloatSlice(data.OBVValues)))
	}

	if indicators.EnableCMF && len(data.CMF20Values) > 0 {
