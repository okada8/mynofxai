...
export function StrategyOptimizer({
  ...
  const [backtestConfig, setBacktestConfig] =
    useState<OptimizationBacktestConfig>({
      symbols: ['BTCUSDT'],
      timeframes: ['15m'],
  ...
  const handleStart = async () => {
    ...
    try {
        ...
        try {
          ...
          if (
            ...
            if (s.status === 'completed') {
              notify.success(
                language === 'zh' ? '优化完成' : 'Optimization completed'
              )
            } else if (s.status === 'failed') {
              notify.error(
                language === 'zh'
                  ? '优化失败: ' + s.error
                  : 'Optimization failed: ' + s.error
              )
            }
          }
        } catch (e) {
      ...
      notify.error(
        language === 'zh' ? '启动优化失败' : 'Failed to start optimization'
      )
      setIsOptimizing(false)
    }
  }
  ...
  const handleApplyParams = () => {
    ...
    const bestParams =
      status?.progress?.best_genes || status?.best_individual?.parameters
          ...
          <div className="p-4 rounded-lg bg-nofx-bg border border-nofx-gold/20 animate-fade-in">
            ...
            <div className="w-full h-1.5 bg-nofx-bg-dark rounded-full overflow-hidden mb-4">
              <div
                ...
                style={{
                  width: `${((status.progress?.generation || 0) / gaConfig.generations) * 100}%`,
                }}
              />
            </div>
              ...
              <div className="border-t border-nofx-gold/10 pt-2 mt-2 space-y-1">
                <div className="text-xs text-nofx-text flex justify-between items-center">
                  ...
                  <span className="text-green-400 font-mono font-bold">
                    {status.progress.best_fitness.toFixed(4)}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-nofx-gold/5">
                  <div className="flex flex-col items-center p-1.5 rounded bg-nofx-bg-lighter border border-nofx-gold/10">
                    <span className="text-[10px] text-nofx-text-muted mb-0.5">
                      {language === 'zh' ? '总收益' : 'Profit'}
                    </span>
                    <span
                      className={`text-xs font-bold font-mono ${status.best_individual?.metrics?.total_return_pct && status.best_individual.metrics.total_return_pct >= 0 ? 'text-nofx-success' : 'text-nofx-danger'}`}
                    >
                      {status.best_individual?.metrics?.total_return_pct
                        ? `${status.best_individual.metrics.total_return_pct.toFixed(2)}%`
                        : '-'}
                    </span>
                  </div>
                  <div className="flex flex-col items-center p-1.5 rounded bg-nofx-bg-lighter border border-nofx-gold/10">
                    <span className="text-[10px] text-nofx-text-muted mb-0.5">
                      {language === 'zh' ? '夏普' : 'Sharpe'}
                    </span>
                    <span className="text-xs font-bold font-mono text-nofx-gold">
                      {status.best_individual?.metrics?.sharpe_ratio
                        ? status.best_individual.metrics.sharpe_ratio.toFixed(2)
                        : '-'}
                    </span>
                  </div>
                  <div className="flex flex-col items-center p-1.5 rounded bg-nofx-bg-lighter border border-nofx-gold/10">
                    <span className="text-[10px] text-nofx-text-muted mb-0.5">
                      {language === 'zh' ? '回撤' : 'MaxDD'}
                    </span>
                    <span className="text-xs font-bold font-mono text-nofx-danger">
                      {status.best_individual?.metrics?.max_drawdown_pct
                        ? `${status.best_individual.metrics.max_drawdown_pct.toFixed(2)}%`
                        : '-'}
                    </span>
                  </div>
                </div>
            ...
            {status.status === 'completed' && status.progress?.best_genes && (
              <div className="mt-4 p-3 rounded bg-green-500/10 border border-green-500/20">
                ...
                <div className="grid grid-cols-2 gap-2 mb-3">
                  {Object.entries(status.progress.best_genes).map(
                    ([key, val]) => (
                      <div key={key} className="text-xs flex justify-between">
                        <span
                          className="text-nofx-text-muted truncate mr-2"
                          title={key}
                        >
                          {key.split('.').pop()}:
                        </span>
                        <span className="text-nofx-text font-mono">
                    ...
                    <button
                      onClick={() => {
                        const bestParams =
                          ...
                          status?.best_individual?.parameters
                        if (bestParams) {
            ...
            <div className="p-3 bg-nofx-bg space-y-3">
              {genes.map((gene, idx) => (
                <div key={idx} className="flex items-center gap-2 text-xs">
                  ...
                  />
                  <span
                    className="flex-1 text-nofx-text truncate"
                    title={gene.path}
                  >
                    {gene.name}
                  </span>
            ...
            <div className="p-3 bg-nofx-bg space-y-3">
              ...
              <div>
                ...
                <div className="text-[10px] text-nofx-text-muted mt-1 flex items-center gap-1">
                  ...
                  {new Date(
                    backtestConfig.start_time
                  ).toLocaleDateString()} -{' '}
                  {new Date(backtestConfig.end_time).toLocaleDateString()}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Object.entries(status.progress.best_genes).map(
                    ([key, val]) => (
                      <div
                        key={key}
                        className="flex justify-between items-center p-2 rounded bg-black/20 border border-nofx-gold/5"
                      >
                        <span
                          className="text-xs text-nofx-text truncate max-w-[180px]"
                          title={key}
                        >
                          {key.split('.').pop()}
                        </span>
