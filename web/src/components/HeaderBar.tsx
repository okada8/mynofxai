      ...
      <div className="flex items-center justify-between h-16 px-4 sm:px-6 max-w-[1920px] mx-auto">
        ...
        <div className="hidden md:flex items-center justify-between flex-1 ml-8">
          ...
          <div className="flex items-center gap-2">
            ...
            {(() => {
              ...
              const navTabs: { page: Page; path: string; label: string; requiresAuth: boolean }[] = [
                ...
                { page: 'indicators', path: '/indicators', label: language === 'zh' ? '指标' : 'Indicators', requiresAuth: true },
                ...
                { page: 'backtest', path: '/backtest', label: '回测', requiresAuth: true },
                { page: 'tasks', path: '/tasks', label: language === 'zh' ? '定时任务' : 'Tasks', requiresAuth: true },
                { page: 'system', path: '/system-monitor', label: language === 'zh' ? '系统监控' : 'System', requiresAuth: true },
              ]
          ...
          <div className="flex items-center gap-4">
            {/* Audit Logs Button (Replaces Social Links) */}
            <div className="flex items-center gap-1">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-all hover:bg-white/5 ${
                  currentPage === 'audit' 
                    ? 'text-nofx-gold bg-nofx-gold/10' 
                    : 'text-nofx-text-muted hover:text-white'
                }`}
                title={language === 'zh' ? '审计日志' : 'Audit Logs'}
              >
