...
export function ExchangeConfigModal({
  ...
  const [polymarketRpcUrl, setPolymarketRpcUrl] = useState(
    'https://polygon-rpc.com'
  )

  // Other state
  const [secureInputTarget, setSecureInputTarget] = useState<
    null | 'hyperliquid' | 'aster' | 'lighter' | 'polymarket'
  >(null)
  ...
  > = {
    ...
    indodax: { url: 'https://indodax.com/register', hasReferral: false },
  }
  ...
  useEffect(() => {
    if (editingExchangeId && selectedExchange) {
      ...
      setPolymarketRpcUrl(
        selectedExchange.polymarketRpcUrl || 'https://polygon-rpc.com'
      )
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (
        ...
        if (
          !polymarketWalletAddr.trim() ||
          !polymarketPrivateKey.trim() ||
          !apiKey.trim() ||
          !secretKey.trim() ||
          !passphrase.trim()
        )
                ...
                >
                  <div>
                    ...
                    <div className="text-xs" style={{ color: '#848E9C' }}>
                      {language === 'zh'
                        ? '使用模拟盘进行无风险测试'
                        : 'Use paper trading for risk-free testing'}
                    </div>
                  </div>
                  <button
                    type="button"
                    ...
                    style={{ background: testnet ? '#F0B90B' : '#2B3139' }}
                  >
                    <span
                      className={`${testnet ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform rounded-full bg-white transition-transform`}
                    />
                  </button>
                </div>
              )}
              ...
              {currentExchangeType === 'polymarket' && (
                <>
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs" style={{ color: '#848E9C' }}>
                          {language === 'zh'
                            ? '需要 Polygon 钱包地址和私钥进行交易'
                            : 'Requires Polygon wallet address and private key'}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={polymarketWalletAddr}
                      onChange={(e) => setPolymarketWalletAddr(e.target.value)}
                      placeholder={
                        language === 'zh'
                          ? '输入 Polygon 钱包地址'
                          : 'Enter Polygon wallet address'
                      }
                      className="w-full px-4 py-3 rounded-xl"
                      style={{
                        background: '#0B0E11',
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => setSecureInputTarget('polymarket')}
                        className="text-xs underline"
                        style={{ color: '#3B82F6' }}
                      >
                        {t('secureInputButton', language)}
                  ...
                  <div className="p-4 rounded-xl mt-4" style={{ background: 'rgba(240, 185, 11, 0.1)', border: '1px solid rgba(240, 185, 11, 0.2)' }}>
                    <div className="text-sm font-semibold mb-2" style={{ color: '#F0B90B' }}>
                      {language === 'zh' ? 'L2 API 认证信息' : 'L2 API Credentials'}
                    </div>
                    <div className="text-xs" style={{ color: '#848E9C' }}>
                      {language === 'zh' ? '用于 Polymarket CLOB 交易认证' : 'Required for Polymarket CLOB trading'}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      value={polymarketRpcUrl}
                      onChange={(e) => setPolymarketRpcUrl(e.target.value)}
                    ...
                    <div className="flex gap-2 mt-1">
                      <button
                        type="button"
                        onClick={() =>
                          setPolymarketRpcUrl('https://polygon-rpc.com')
                        }
                  ...
                  >
                    <div className="flex items-center gap-2 text-sm text-yellow-400">
                      ...
                      <span>
                        {language === 'zh'
                          ? '安全提示：私钥请妥善保管，建议使用专用交易钱包'
                          : 'Security Warning: Keep your private key safe. Use a dedicated wallet.'}
                      </span>
                    </div>
                  </div>
                </>
