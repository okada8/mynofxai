        ...
        <div className="px-6 pb-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 16rem)' }}>
          ...
          {(currentStep === 1 || editingExchangeId) && selectedTemplate && (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {/* Testnet Toggle */}
              {(currentExchangeType === 'binance' || currentExchangeType === 'bybit' || currentExchangeType === 'hyperliquid') && (
                <div className="flex items-center justify-between p-4 rounded-xl mb-4" style={{ background: '#0B0E11', border: '1px solid #2B3139' }}>
                  <div>
                    <div className="text-sm font-semibold" style={{ color: '#EAECEF' }}>
                      {language === 'zh' ? '测试网模式 (Testnet)' : 'Testnet Mode'}
                    </div>
                    <div className="text-xs" style={{ color: '#848E9C' }}>
                      {language === 'zh' ? '使用模拟盘进行无风险测试' : 'Use paper trading for risk-free testing'}
                    </div>
                  </div>
                  <button
                    type="button"
