...
function isPerpDexExchange(exchangeType: string | undefined): boolean {
  ...
  const perpDexTypes = ['hyperliquid', 'lighter', 'aster', 'polymarket']
...
function getWalletAddress(exchange: Exchange | undefined): string | undefined {
  ...
  switch (type) {
    ...
    case 'polymarket':
      return exchange.polymarketWalletAddr
    default:
      return undefined
  }
}
...
function CapitalAllocationModal({
  ...
  const { data, isLoading } = useSWR(
    isOpen ? 'capital-allocation' : null,
    api.getCapitalAllocation
  )
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {Object.entries(data.target_allocation || {}).map(
                    ([exchange, targetPct]: [string, any]) => {
                      const currentBal = data.current_balances?.[exchange] || 0
                      const currentPct =
                        data.total_capital > 0
                          ? currentBal / data.total_capital
                          : 0
                      ...
                      return (
                        <div
                          key={exchange}
                          className="p-3 bg-[#2B3139]/30 rounded-lg"
                        >
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {data.rebalance_plan.map((item: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex items-center justify-between p-3 bg-[#2B3139]/50 rounded text-sm"
                      >
                        <div className="flex items-center gap-2">
