...
import { AlertTriangle, CheckCircle, ShieldAlert, TrendingDown, Percent, Activity } from 'lucide-react'
...
export function RiskMonitorPanel({ riskState, totalEquity, language }: RiskMonitorPanelProps) {
    ...
    const getLevelColor = (level: string) => {
        switch (level?.toLowerCase()) {
            case 'safe': return 'text-nofx-green border-nofx-green/30 bg-nofx-green/10'
            case 'caution': return 'text-nofx-gold border-nofx-gold/30 bg-nofx-gold/10'
            case 'danger': return 'text-nofx-red border-nofx-red/30 bg-nofx-red/10'
    ...
    return (
        <div className="nofx-glass p-6 rounded-lg border border-white/5 animate-slide-in relative overflow-hidden group">
            {/* Background Glow */}
            <div className={`absolute top-0 right-0 p-20 opacity-5 blur-3xl rounded-full transition-colors duration-500 ${level?.toLowerCase() === 'safe' ? 'bg-nofx-green' : level?.toLowerCase() === 'caution' ? 'bg-nofx-gold' : 'bg-nofx-red'}`} />
            ...
            <div className="flex items-center justify-between mb-6 relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-nofx-text-main">{t('title')}</h2>
                        <div className="flex items-center gap-2 text-xs font-mono mt-1">
                            <span className="text-nofx-text-muted">SYSTEM_RISK_CHECK::ACTIVE</span>
                            <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${level?.toLowerCase() === 'safe' ? 'bg-nofx-green' : level?.toLowerCase() === 'caution' ? 'bg-nofx-gold' : 'bg-nofx-red'}`} />
                        </div>
                    </div>
                </div>

                <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border ${levelColorClass}`}>
                    {levelIcon}
                    <span className="font-bold uppercase tracking-wider">{t(level?.toLowerCase() || 'unknown')}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 relative z-10">
                ...
                <RiskMetric
                    ...
                    color={var_pct > 5 ? 'text-nofx-red' : var_pct > 2 ? 'text-nofx-gold' : 'text-nofx-green'}
                />
                ...
                <RiskMetric
                    ...
                    color={Math.abs(max_drawdown) > 20 ? 'text-nofx-red' : Math.abs(max_drawdown) > 10 ? 'text-nofx-gold' : 'text-nofx-text-main'}
                />
                ...
                <RiskMetric
                    ...
                    color={leverageRatio > 3 ? 'text-nofx-red' : leverageRatio > 1.5 ? 'text-nofx-gold' : 'text-nofx-text-main'}
                />
                ...
                {/* Margin Utilization */}
                 <RiskMetric
                    ...
                    icon={<Percent className="w-4 h-4" />}
                    color={utilization > 80 ? 'text-nofx-red' : utilization > 50 ? 'text-nofx-gold' : 'text-nofx-green'}
                />
            </div>
