    ...
    <div className="rounded-lg p-4 border border-gray-800 bg-gray-900/50 hover:border-purple-500/50 transition-all">
      <div className="flex justify-between items-start mb-3">
        <div>
          ...
          <div className="flex gap-2 text-xs text-gray-400">
            <span>Liquidity: ${parseFloat(market.liquidity || '0').toLocaleString()}</span>
            <span>•</span>
            <span>Vol: ${parseFloat(market.volume24h || '0').toLocaleString()}</span>
          </div>
        </div>
        <button
      ...
      <div className="space-y-2">
        {outcomes.slice(0, 2).map((outcome: any, idx: number) => {
          ...
          return (
            <div key={outcome.id || idx} className="relative h-8 bg-gray-800 rounded overflow-hidden flex items-center px-3">
              <div 
