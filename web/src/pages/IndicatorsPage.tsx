    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="text-xs text-gray-500 bg-[#1E2329] px-3 py-1 rounded border border-[#2B3139]">
          {language === 'zh'
            ? '数据来源: Alternative.me & Binance'
            : 'Source: Alternative.me & Binance'}
        </div>
      </div>
        ...
        >
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-4">
              <div
                className={`text-6xl font-black mb-2 ${getFngColor(parseInt(fngData.value))}`}
              >
                {fngData.value}
              </div>
              <div
                className={`text-xl font-medium px-4 py-1 rounded-full bg-opacity-10 ${getFngColor(
                  parseInt(fngData.value)
                ).replace(
                  'text-',
                  'bg-'
        ...
        >
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="mt-2 pt-4 border-t border-[#2B3139] text-xs text-gray-500">
              {language === 'zh'
                ? '基于币安合约实时持仓数据'
                : 'Based on Binance Futures real-time open interest'}
            </div>
          </div>
        </motion.div>
