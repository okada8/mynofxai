...
function App() {
  ...
  useEffect(() => {
    const handleRouteChange = () => {
      ...
      } else if (path === '/indicators' || hash === 'indicators') {
      ...
      } else if (path === '/audit-logs' || hash === 'audit') {
  ...
  const { data: status } = useSWR<SystemStatus>(
    currentPage === 'trader' && selectedTraderId
      ...
      : null,
    api.getStatus ? () => api.getStatus(selectedTraderId) : null,
    {
  ...
  const { data: account } = useSWR<AccountInfo>(
    currentPage === 'trader' && selectedTraderId
      ...
      : null,
    api.getAccount ? () => api.getAccount(selectedTraderId) : null,
    {
      refreshInterval: 15000, // 15秒刷新（配合后端15秒缓存）
      revalidateOnFocus: false, // 禁用聚焦时重新验证，减少请求
      dedupingInterval: 10000, // 10秒去重，防止短时间内重复请求
  ...
  const { data: positions } = useSWR<Position[]>(
    currentPage === 'trader' && selectedTraderId
      ...
      : null,
    api.getPositions ? () => api.getPositions(selectedTraderId) : null,
    {
      refreshInterval: 15000, // 15秒刷新（配合后端15秒缓存）
      revalidateOnFocus: false, // 禁用聚焦时重新验证，减少请求
      dedupingInterval: 10000, // 10秒去重，防止短时间内重复请求
  ...
  const { data: stats } = useSWR<Statistics>(
    currentPage === 'trader' && selectedTraderId
      ...
      : null,
    api.getStatistics ? () => api.getStatistics(selectedTraderId) : null,
    {
