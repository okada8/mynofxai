...
func (s *Store) initTables() error {
	...
	if err := s.gdb.AutoMigrate(&APIKey{}); err != nil {
		return fmt.Errorf("failed to initialize api_key tables: %w", err)
	}
	if err := s.gdb.AutoMigrate(&Task{}); err != nil {
		return fmt.Errorf("failed to initialize task tables: %w", err)
	}
...
func (s *Store) APIKey() *APIKeyStore {
	...
	defer s.mu.Unlock()
	if s.apiKey == nil {
		s.apiKey = NewAPIKeyStore(s.gdb)
	}
	return s.apiKey
}

// Task gets task storage
func (s *Store) Task() *TaskStore {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.task == nil {
		s.task = &TaskStore{db: s.gdb}
	}
	return s.task
}
