...
func ToChecksumAddress(address string) string {
	...
	for i, c := range addr {
		...
		if nibble >= 8 && c >= 'a' && c <= 'f' {
			// Fix G115: c is rune (int32), safe to cast to byte since it's 'a'-'f'
			result.WriteByte(byte(c - 32)) // Convert to uppercase
		} else {
			// Fix G115: c is rune (int32), safe to cast to byte since it's hex char
			result.WriteByte(byte(c & 0xff))
		}
	}
