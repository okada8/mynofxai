...
func NewAutoTrader(config AutoTraderConfig, st *store.Store, userID string) (*AutoTrader, error) {
	...
	case "bybit":
		logger.Infof("🔵 [%s] Using Bybit USDT Perpetual trading", config.Name)
		trader = bybit.NewBybitTrader(config.BybitAPIKey, config.BybitSecretKey, config.BybitTestnet)
	case "okx":
...
func (at *AutoTrader) Run() error {
	...
	go func() {
		msg := fmt.Sprintf("🚀 *AI Trader Started*\n"+
			"Name: `%s`\n"+
			"Model: `%s`\n"+
			"Exchange: `%s`\n"+
			"Initial Balance: `%.2f` USDT",
			at.name, at.aiModel, at.exchange, at.initialBalance)
		_ = notification.SendTelegramMessage(msg)
	}()
...
func (at *AutoTrader) Stop() {
	...
	// Send notification (non-blocking)
	go func() {
		msg := fmt.Sprintf("⏹ *AI Trader Stopped*\n"+
			"Name: `%s`\n"+
			"Exchange: `%s`",
			at.name, at.exchange)
		_ = notification.SendTelegramMessage(msg)
...
func (at *AutoTrader) checkPositionDrawdown() {
	...
	// Default parameters (legacy behavior)
	minProfitToActivate := 5.0 // Activate when profit > 5%
	maxDrawdownPct := 40.0     // Close when giving back 40% of profit
	isEnabled := true          // Default enabled

	// Override with strategy config if available
	if config != nil {
		// 1. Check Trailing Stop Config
		if config.TrailingStopConfig != nil {
			isEnabled = config.TrailingStopConfig.Enabled
			if config.TrailingStopConfig.PeakDecline.Enabled {
				if config.TrailingStopConfig.PeakDecline.ActivationPct > 0 {
					minProfitToActivate = config.TrailingStopConfig.PeakDecline.ActivationPct
				}
			}
		}

		// 2. Check Callback Rate
		if config.RiskControl.TrailingStopCallbackRatePct > 0 {
			maxDrawdownPct = config.RiskControl.TrailingStopCallbackRatePct
		}

		// 3. Check CloseWhenDrawdownFromPeakPct (alternative config field)
		if config.RiskControl.CloseWhenDrawdownFromPeakPct > 0 {
			maxDrawdownPct = config.RiskControl.CloseWhenDrawdownFromPeakPct
		}

		// 4. Check CloseWhenProfitExceedsPct (alternative activation)
		if config.RiskControl.CloseWhenProfitExceedsPct > 0 {
			minProfitToActivate = config.RiskControl.CloseWhenProfitExceedsPct
		}
	}
	...
	for _, pos := range positions {
		...
		if !exists {
			...
			// Update peak cache
			at.UpdatePeakPnL(symbol, side, currentPnLPct)
			// Refresh local variable if updated
		...
		if currentPnLPct > minProfitToActivate && drawdownPct >= maxDrawdownPct {
			logger.Infof("🚨 Trailing Stop Triggered: %s %s | Current: %.2f%% | Peak: %.2f%% | Drawdown: %.2f%% (Limit: %.1f%%)",
				symbol, side, currentPnLPct, peakPnLPct, drawdownPct, maxDrawdownPct)

			// Send notification (non-blocking)
			go func() {
				msg := fmt.Sprintf("🚨 *Trailing Stop Triggered*\n"+
					"Symbol: `%s`\n"+
					"Side: *%s*\n"+
					"Current Profit: `%.2f%%`\n"+
					"Peak Profit: `%.2f%%`\n"+
		...
		} else if currentPnLPct > minProfitToActivate {
			// Record situations close to close position condition (for debugging)
			logger.Infof("📊 Trailing monitor: %s %s | Profit: %.2f%% | Peak: %.2f%% | Drawdown: %.2f%% | Activ: %.1f%%",
				symbol, side, currentPnLPct, peakPnLPct, drawdownPct, minProfitToActivate)
		}
	}
}
...
func (at *AutoTrader) recordAndConfirmOrder(orderResult map[string]interface{}, symbol, action string, quantity float64, price float64, leverage int, entryPrice float64) {
	...
	case "binance", "lighter", "hyperliquid", "bybit", "okx", "bitget", "aster", "kucoin", "gate":
		// Send notification manually since CreateFill is skipped
		go func() {
			// Determine side
			side := "BUY"
			...
			// Calculate estimated PnL for close orders
			var pnl float64
			...
			msg := fmt.Sprintf("🔔 *Trade Executed*\n"+
				"Symbol: `%s`\n"+
				"Side: *%s*\n"+
				"Price: `%.6f`\n"+
				"Qty: `%.6f`\n"+
				"PnL: `%.2f` USDT",
				symbol, side, price, quantity, pnl)
			_ = notification.SendTelegramMessage(msg)
		}()
...
// SendHourlyReport sends hourly position report
func (at *AutoTrader) SendHourlyReport() {
	// 1. Get Balance
	balance, err := at.trader.GetBalance()
	if err != nil {
		logger.Errorf("Failed to get balance for hourly report: %v", err)
		return
	}
	...
	if eq, ok := balance["totalEquity"].(float64); ok {
		totalEquity = eq
	}
	if avail, ok := balance["availableBalance"].(float64); ok {
		availableBalance = avail
	...
	// Fallback logic for totalEquity if it's 0 (similar to GetAccountInfo)
	if totalEquity == 0 {
		if wallet, ok := balance["totalWalletBalance"].(float64); ok {
			totalEquity = wallet + totalUnrealizedPnL
		}
	}
	...
	positions, err := at.trader.GetPositions()
	if err != nil {
		logger.Errorf("Failed to get positions for hourly report: %v", err)
		return
	}

	// Filter out zero positions
	var activePositions []map[string]interface{}
	for _, pos := range positions {
		if amt, ok := pos["positionAmt"].(float64); ok && amt != 0 {
	...
	// 3. Format Message
	// Use UTC+7 as requested
	loc := time.FixedZone("UTC+7", 7*60*60)
	timeStr := time.Now().In(loc).Format("2006-01-02 15:04:05 +07")

	msg := fmt.Sprintf("🕐 NOFX每小时持仓报告 (%s)\n\n", timeStr)
	msg += "📊 *持仓概览:*\n"
	msg += fmt.Sprintf("持仓数量: %d\n", len(activePositions))
	msg += fmt.Sprintf("总权益: $%.8f\n", totalEquity)
	msg += fmt.Sprintf("可用余额: $%.8f\n", availableBalance)
	msg += fmt.Sprintf("未实现盈亏: $%.4f\n\n", totalUnrealizedPnL)
	...
	if len(activePositions) > 0 {
		...
		for _, pos := range activePositions {
			...
			margin := 0.0
			if leverage > 0 {
				margin = (quantity * markPrice) / float64(leverage)
			}
			...
			priceChangePct := 0.0
			if entryPrice > 0 {
				// For display, usually we want the asset price change relative to entry
				priceChangePct = (markPrice - entryPrice) / entryPrice * 100
			}

			// Calculate Stop Loss Distance
			slDistanceStr := "N/A"
			// Use GetOpenOrders from AutoTrader which calls underlying trader
			openOrders, err := at.GetOpenOrders(symbol)
			if err == nil {
				for _, o := range openOrders {
					// Check for stop loss order
					// Usually Type contains STOP
					isStop := strings.Contains(strings.ToUpper(o.Type), "STOP")
					if isStop && o.StopPrice > 0 {
						dist := math.Abs(markPrice-o.StopPrice) / markPrice * 100
						slDistanceStr = fmt.Sprintf("%.2f%%", dist)
						break
					}
				}
			}

			msg += fmt.Sprintf("• %s %s: %.4f @ %.6f\n", symbol, side, quantity, entryPrice)
			msg += fmt.Sprintf("当前: %.6f, 浮盈: %.2f%% ($%.2f)\n", markPrice, pnlPct, unrealizedPnl)
			msg += fmt.Sprintf("价格变化: %.2f%%, 杠杆: %dx, 保证金: %.2f\n", priceChangePct, leverage, margin)
			msg += fmt.Sprintf("止损距离: %s\n\n", slDistanceStr)
		}
	}
